const Xiukai = {
   ...
   ,W_Skill: (character, enemy) => {
       ...
       if (character.weapon && w >= 0) {
           const damage = calcSkillDamage(character, enemy, 60 + w * 40 + character.max_hp * (0.02 + w * 0.005), 0.5, 1);
           const cost = 30 + w * 15;
       ...
       if (character.weapon) {
           ...
           for (let i = 0; i < combo.length; i++) {
               ...
               if (c === 'a') {
                   ...
                   if (w >= 0) {
                       damage += calcSkillDamage(character, enemy, 60 + w * 40 + character.max_hp * (0.02 + w * 0.005), 0.5, 1);
                       cc = true;
                       life -= 30 + w * 15;
                   }
               } else if (c === 'e' || c === 'E') {
